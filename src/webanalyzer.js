...
const WebAnalyzer = {
    ...
    init: (data_files) => {
        for (const file of data_files) {
            ...
            WebAnalyzer.technologies = Object.assign(Object.assign({}, WebAnalyzer.technologies), technologiesFromFile);
        }
    ...
    fetchURL: (url) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            return {
                source_code: yield response.text(),
    ...
    parseSourceCode: (source_code) => {
        const dom = new jsdom_1.JSDOM(source_code);
        ...
        //  extract meta data
        const meta = {};
        Array.from(doc.querySelectorAll("meta")).forEach((metaElement) => {
    ...
    detectPatterns: (site_data, url_data) => {
        const detectedTechnologies = [];
        const visited = new Set();
        const detect = (techName) => {
            if (visited.has(techName))
                return;
            ...
            if (!techData)
                return;
            let detected = false;
            let detectedUsing = null;
            // Match JavaScript keys
            if (techData.js &&
                site_data.js.some((script) => Object.keys(techData.js).some((key) => WebAnalyzer.matchPattern(script, key)))) {
                detected = true;
                detectedUsing = "js";
            }
            // Match scriptSrc
            if (!detected && techData.scriptSrc) {
                const patterns = Array.isArray(techData.scriptSrc)
                    ? techData.scriptSrc
                    : [techData.scriptSrc];
                if (patterns.some((pattern) => site_data.scriptSrc.some((src) => WebAnalyzer.matchPattern(src, pattern)))) {
                    detected = true;
            ...
            if (!detected && techData.cookies) {
                if (Object.keys(techData.cookies).some((key) => url_data.cookies.includes(key))) {
                    detected = true;
            ...
            if (!detected && techData.headers) {
                if (Object.entries(techData.headers).some(([key, val]) => {
                    const headerValue = url_data.headers.get(key.trim().replace(":", ""));
            ...
            if (!detected && techData.meta && site_data.meta) {
                if (Object.entries(techData.meta).some(([key, val]) => {
