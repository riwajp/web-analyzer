...
(async () => {
  ...
  try {
    ...
    if (blockedSites.length > 0) {
      fs.writeFileSync(
        "src/sample-runs/results/blocked-sites.json",
        JSON.stringify(blockedSites, null, 2),
        "utf-8"
      );
      console.log(
        `${blockedSites.length} blocked sites saved to blocked-sites.json`
      );
    }
    ...
    if (notBlockedSites.length > 0) {
      fs.writeFileSync(
        "src/sample-runs/results/not-blocked-sites.json",
        JSON.stringify(notBlockedSites, null, 2),
        "utf-8"
      );
      console.log(
        `${notBlockedSites.length} not-blocked sites saved to not-blocked-sites.json`
      );
    }
    ...
    if (highConfidenceDetections.length > 0) {
      fs.writeFileSync(
        "src/sample-runs/results/high-confidence-detections.json",
